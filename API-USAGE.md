...
Updated: 2026-09-25. Applies to this NAS fork. `192.168.1.100` and `NAS-HOST` are examples; replace them with your own addresses.
...
The flag must be a JSON boolean, not a string. Combining `true` with a nonempty `conversation_id` returns HTTP 400 before browser mutation. Omitted or `false` continues an explicit ID; without an ID, pooled REST starts fresh and singleton mode retains legacy selection. An unavailable explicit ID returns an error instead of silently creating a new chat. This flag does not change account-level settings.
...
Do not resend the entire history while also continuing the original conversation, or context will be duplicated. Keep separate IDs for separate business conversations. Pooled REST always starts fresh without a `conversation_id`. Only singleton mode may implicitly reuse the last chat. Explicitly create or continue each business conversation; never rely on shared last-chat state.

Start acceptance with serial requests, then allow distinct conversations up to `/health.rest_pool.size` in parallel. Keep sends within one business conversation sequential. See [client migration](docs/REST-CONCURRENCY.md). Do not manually switch chats or send messages in noVNC while a request is running.
