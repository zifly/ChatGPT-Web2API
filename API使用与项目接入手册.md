...
| 功能 | 当前情况 |
|---|---|
| 普通文本请求 | 已通过最小 OK 测试及固定结构化业务样本；不代表表中所有任务类型均已实测 |
| 多轮文本 | 代码支持历史 messages 和自定义 conversation_id；用法见第 5 节 |
| 流式 SSE | 保留 SSE 格式，正文等待最终核对后发送，不再实时逐字输出；NAS 路径尚未单独验收，接入先关闭 |
...
这是服务端配置，不是每个 API 请求中的参数。公开配置默认 reconciled；本次 NAS 真实业务验收使用 backend。backend 模式要求正式会话 ID 和可信的本轮匹配，无法确认、空白或超时都明确失败。默认模式的页面回退可读取孤立代码块正文，但拒绝混合正文/代码及多代码块场景的页面回退。

`stream: true` 仍为 SSE，正文在核对完成后发送。客户端和反向代理必须允许等待完整生成；收到错误结束事件不能当作成功。模型本身仍可能输出非法 JSON 或错误业务结论。

## 10. 本手册的验证范围

接口字段、会话分支和错误码已对照源码。回复完整性与协议模式通过 146 项相关离线回归；本机一次合成 JSON 请求逐字一致。NAS 上 3 个固定业务样本进行了两轮测试，共 6 请求、10 组结果通过 JSON/schema/case ID/候选池校验，第二轮经真实审查页面触发，结果保持待审核，未修改正式作品关联。

尚无这些业务请求同轮协议原文的独立逐字对照；主音乐库管理员登录后的结果展示未验证。长文本、多轮、高并发、长期稳定性和 NAS SSE 路径待验收。业务语义仍需人工或业务规则审查。本文代码示例应在调用项目的网络和依赖环境中单独验证，SDK 示例未单独执行。
