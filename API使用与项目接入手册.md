...
**实时进度（2026-09-26）：** 非流式和 SSE 都可传入新的 UUID 作为 `progress_id`，等待期间用相同 API Key 每秒查询 `GET /v1/requests/{progress_id}`。项目端可显示排队、打开会话、输入、上传和等待回复；最终诊断也附带阶段耗时。具体修改和错误处理见[进度接入说明](docs/REQUEST-PROGRESS.md)，查询失败不能触发聊天重发。

## 1. 接入参数

**使用统计页面：** 更新服务后打开 `/stats`，用服务 API Key 查看该密钥的历史调用、耗时和接口正文流量。记录会持久化，进度轮询不计入聊天调用次数；项目也可查询 `/v1/stats`，详见[统计口径与保存说明](docs/USAGE-DASHBOARD.md)。
