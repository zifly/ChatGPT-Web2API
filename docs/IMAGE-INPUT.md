...
`POST /v1/chat/completions` 支持在最后一条 user 消息中提交文字和图片。程序将图片交给 ChatGPT 网页的上传控件，确认上传完成、预览已解码且发送按钮可用，然后发送问题。返回值仍是文字；没有图片生成、编辑或下载接口。MCP 暂未增加图片参数。

2026-09-25 更新支持新版 ProseMirror 输入框、按 `image/*` 定位的图片控件和按钮式附件卡片。旧版服务器预览图仍可作为确认依据；新版网页保留本地预览时，必须匹配本次随机文件名和服务器文件 ID，并读取成功响应中的 `file.processing.completed` 事件。仅有本地缩略图、HTTP 200 或 `file_ready` 事件不算上传完成。捕获按浏览器标签页隔离，不重复上传。

不同会话并发与前端迁移要求见[REST 多会话并发](REST-CONCURRENCY.md)，图片参数不变。
...
### Two-worker REST acceptance / 两路并发验收（2026-09-25）

The updated uploader passed a real blue-image request concurrently with a separate text request, followed by an explicit-ID SSE continuation while the text chat also continued. All four requests passed with isolated and stable conversation IDs. The modern/legacy DOM fixture and 26 upload/acknowledgment tests also passed. Full timings and client requirements are in the [REST concurrency guide](REST-CONCURRENCY.md).
