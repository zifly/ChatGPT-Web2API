...
项目端可在原 JSON/SSE 请求等待时查询[实时进度](REQUEST-PROGRESS.md)，显示当前是排队、打开会话还是等待回复；通过 `progress_id` 关联本次尝试，不能使用一个全局进度变量覆盖不同业务会话。
...
并发更新通过 221 项相关离线测试，其中 23 项为新增池测试，另有 26 项图片上传与服务器确认测试，覆盖 HTTP 两路重叠、同 ID 串行、容量/取消、JSON/SSE/图片参数隔离、失败槽位隔离、账号冷却和启动清理；浏览器行为使用合成替身。其中也包括 5 项新增的新会话地址变化回归和 34 项消息归属选择测试，覆盖严格会话校验及同文不同消息的误匹配防护。后续新会话重试更新增加 14 项用例，相关套件合计 235 项通过；网关和前端仍需自行完成[替换重试验收](NEW-CONVERSATION-RETRY.md#4-验收)。
