...
本表按 2026-09-25 的请求保护、图片输入、导航恢复、REST 并发及新会话重试实现核对。旧镜像可能缺少部分代码或字段；文档更新不会自动更新部署。本表不把 MCP 工具错误当作 REST 契约。
...
5. 关闭 SDK、任务队列和反向代理对聊天请求的原样自动重放。`automatic_retry_allowed=false` 保留此语义，新会话替换策略由独立的 `error.new_conversation_retry` 提供。
6. 429 或 5xx 可由网关放弃旧尝试，在新会话自动重试一次，包括 `unknown` 和 `confirmed`；先占用任务级预算并使旧尝试失效，再等待冷却、重建上下文/图片并发送 `new_conversation:true`，不携带旧 ID。第二次失败停止，400/401 等其他 4xx 及用户取消不自动重试。完整流程见[替换重试约定](NEW-CONVERSATION-RETRY.md)。

这些接口没有提供幂等提交或失败请求去重保证。`Retry-After` 也不是自动重发许可。

`new_conversation_retry` 包含 `allowed`、`mode=new_conversation`、`max_retries` 和 `retry_after_seconds`；它不是跨请求计数器，预算、迟到结果过滤和新 ID 保存由网关执行。没有完整错误体的网络故障可按网关显式策略执行同一流程。放弃旧尝试不要求先确认旧生成完成，也不会删除旧聊天。下方响应示例仅展示所讨论的字段。
...
|---|---|---|---|
| `not_sent` | `false` | 本次请求尚未执行发送动作；可能已经导航、填入文字或上传附件 | 根据错误策略及任务预算决定是否新建重试 |
| `confirmed` | `true` | 本次发送已得到确认，不代表回复成功收集 | 可按策略放弃旧尝试、新建重试；旧生成可能继续 |
| `unknown` | `null` | 已尝试发送，但没有足够证据确认结果 | 保留旧槽位保护，过滤旧结果，按策略使用健康槽位新建重试 |
...
| 429 | `rate_limit_exceeded` | 网页触发限流 | 等待指定冷却后，可在任务的一次预算内新建重试；不能绕过账号限流 |
| 500 | **无 code**，`type=server_error` | 未单独分类的内部异常 | 检查发送状态、发生阶段和同一时间附近的服务日志 |
| 502 | `navigation_failed` | 导航命令明确报告失败 | 检查 `error.navigation`；可按策略新建重试一次 |
| 502 | `navigation_displaced` | 曾到达目标地址，随后连续探测确认页面偏离目标 | 检查登录跳转或其他页面操作；本次导航路径尚未发送 |
| 503 | `browser_unresponsive` | 所需槽位或全部浏览器槽位暂停 | 查看 `/health.rest_pool`；新会话可用健康槽位，无可用槽位时才按策略只读恢复 |
| 503 | `browser_pool_busy` | 队列已满或服务正在关闭 | 退避后可在任务的一次预算内新建重试 |
...
| 504 | `client_disconnected` | 服务端检测到调用方断开连接并停止请求 | 响应策略禁止重试；用户主动取消不能触发自动重试 |
...
| 504 | `reply_timeout` | 无法在回复核对截止时间前取得可信的最终回复 | 网页可能已经回答；可放弃旧尝试、新建重试一次并丢弃旧结果 |
...
故障槽位的人工排障可检查 `/health` 和远程桌面，再按需修复页面/登录、只读恢复。网关替换重试无需等待旧生成完成或先恢复旧槽位；有健康槽位即可使用，无可用槽位时才在一次预算内探测恢复。繁忙或探测失败即停止，不循环恢复，也不为一个失败请求重启共享服务。
