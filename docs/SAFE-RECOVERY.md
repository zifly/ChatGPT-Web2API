...
本说明对应包含安全准备恢复更新的代码。旧镜像没有这些能力；更新文档不等于 NAS 已部署。客户端继续只提交一次聊天 POST，关闭 SDK、任务队列和代理的自动重放。最终错误的 `automatic_retry_allowed` 仍是 `false`。
...
REST 中的限流直接返回 429，不再重复运行包含输入/发送的工厂函数，也不自动消除限流提示。`Retry-After` 向上取整到秒，不授权客户端重发。非 REST 的旧限流帮助函数仍保留有限尝试，但要求等待超过其 cap 时直接返回错误，不将等待截短后重试。
...
This contract requires the safe-preparation-recovery update; documentation alone does not update a deployment. Clients submit one chat POST and disable automatic replay. Final errors retain `automatic_retry_allowed=false`.
