...
async def read_protocol_reply(driver, anchor, timeout: float) -> tuple[str, str, list[dict]]:
    ...
    conversation_id = ""
    # A new-chat route can expose a provisional ID before the server route
    # settles. Only an observed outgoing user UUID lets us safely follow that
    # transition: the selector never falls back to text when this UUID exists.
    provisional = bool(anchor.mode == "fresh_chat"
                       and not anchor.conversation_id_at_capture
                       and anchor.captured_user_message_id)
    candidate_changes = 0
            ...
            while True:
                ...
                if current_id:
                    ...
                    if expected_id and current_id != expected_id:
                        if not provisional or candidate_changes >= 3:
                        ...
                        candidate_changes += 1
                        _record_read_recovery()
                        logger.info("Fresh-chat route changed before the captured user node resolved; retrying anchored read")
                    ...
                    if result.diagnostic.get("user_node"):
                        # Once this exact submitted user node is present, pin
                        # its conversation even if the assistant is unfinished.
                        provisional = False
