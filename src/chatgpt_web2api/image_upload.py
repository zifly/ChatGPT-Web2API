...
from .image_upload_ack import UploadAcknowledgements

logger = logging.getLogger(__name__)

_COMPOSER_JS = (
    f'document.querySelector({json.dumps(COMPOSER_SELECTOR)}) || '
    f'document.querySelector({json.dumps(COMPOSER_FALLBACK_SELECTOR)})'
)
_SEND_JS = (
    f'form.querySelector({json.dumps(SEND_BUTTON_SELECTOR)}) || '
    f'form.querySelector({json.dumps(SEND_BUTTON_FALLBACK_SELECTOR)}) || '
    'form.querySelector(\'button[type="submit"]:not([data-testid="stop-button"])\')'
)

FILE_INPUT_JS = """(() => {
  const composer = __COMPOSER__;
  const form = composer && composer.closest('form');
  if (!form) return null;
  const legacy = form.querySelector('#upload-photos, input[data-testid="upload-photos-input"]');
  if (legacy) return legacy;
  const inputs = [...form.querySelectorAll('input[type="file"]')]
    .filter(e => (e.accept || '').toLowerCase().replace(/\\s/g, '') === 'image/*');
  return inputs.length === 1 ? inputs[0] : null;
})()""".replace('__COMPOSER__', _COMPOSER_JS)

# A local blob preview alone is NOT proof of a completed upload. Require
# a decoded remote preview OR a correlated server processing completion,
# exact filenames and an enabled send.
UPLOAD_STATE_JS = """(() => {
  const composer = __COMPOSER__;
  const form = composer && composer.closest('form');
  if (!form) return JSON.stringify({missing:true});
  const tiles = [...form.querySelectorAll('[role="group"][aria-label], [role="button"][aria-label]')]
  ...
  const ready = __D.names.every(name => {
    ...
    const confirmed = remote || (__D.uploaded || []).includes(name);
    return confirmed && img.complete && img.naturalWidth > 0 &&
...
})()""".replace('__COMPOSER__', _COMPOSER_JS).replace('__SEND__', _SEND_JS)

CLEANUP_JS = """(() => {
  const composer = __COMPOSER__;
  const form = composer && composer.closest('form');
  if (!form) return false;
  for (const tile of form.querySelectorAll('[role="group"][aria-label], [role="button"][aria-label]')) {
...
async def upload_images(driver, images: list[ImageInput], timeout: float = 90) -> list[str]:
    ...
    stage = 'preflight'
    state = {}
    acknowledgements = UploadAcknowledgements(driver, names)
    try:
        async with asyncio.timeout(timeout):
            ...
            with tempfile.TemporaryDirectory(prefix='w2a-images-') as directory:
                ...
                while True:
                    try:
                        uploaded = await acknowledgements.confirmed_names()
                        state = json.loads(await driver._js_with_data_strict(
                            UPLOAD_STATE_JS, {'names': names, 'uploaded': uploaded}, timeout=5))
                    except TimeoutError:
                        stable = 0
