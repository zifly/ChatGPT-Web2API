...
回复完整性修复与 backend 模式此前通过 146 项相关离线回归；本机单次合成 JSON 测试逐字一致。本次图片、会话控制与启动检查共通过 145 项相关测试。在一台 amd64 NAS 上，使用合成图片和虚构测试编号验证了新建、图片识别、SSE 追问与按 ID 切换。详见[图片验证记录](docs/IMAGE-INPUT.md)。
