...
可在 `.env` 中通过 `W2A_PIP_INDEX_URL` 更换 Python 镜像源。Debian 及安全更新使用官方源，如需修改须调整 Dockerfile。
...
当前服务的登录等待有截止时间，请及时完成网页登录；若启动因登录超时退出，完成登录后再启动服务。登录前 API 尚未就绪；桌面页面可用不等于聊天 API 已就绪。
