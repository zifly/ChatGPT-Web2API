...
@pytest.mark.asyncio
async def test_fresh_route_transition_requires_exact_captured_user(monkeypatch):
    from chatgpt_web2api.turn_anchor import select_text_for_turn
    d = driver_for(monkeypatch, [], "")
    anchor = TurnAnchor(sent_text="request", mode="fresh_chat",
                        captured_user_message_id="submitted-user")

    def result(user_id, text):
        return select_text_for_turn({"nodes": {
            user_id: {"id": user_id, "role": "user", "text": "request", "children": ["reply"]},
            "reply": {"id": "reply", "role": "assistant", "text": text,
                      "content_type": "text", "end_turn": True, "parent": user_id},
        }}, anchor)

    # Identical prompt text in a different conversation must never be accepted.
    wrong = result("different-user", "unrelated reply")
    assert wrong.status == "not_ready"
    d._conversation_id_from_url = AsyncMock(side_effect=["provisional", "server"])
    d._fetch_text_for_turn = AsyncMock(side_effect=[wrong, result("submitted-user", "correct reply")])
    assert await read_protocol_reply(d, anchor, 1) == ("server", "correct reply", [])
    assert [call.args[0] for call in d._fetch_text_for_turn.await_args_list] == ["provisional", "server"]
    assert all(call.args[1] is anchor for call in d._fetch_text_for_turn.await_args_list)


@pytest.mark.asyncio
@pytest.mark.parametrize("mode", ["fresh_chat", "existing_conversation"])
async def test_captured_uuid_never_overrides_explicit_conversation(monkeypatch, mode):
    d = driver_for(monkeypatch, [], "wrong", conv_id="other")
    anchor = TurnAnchor(sent_text="request", mode=mode,
                        conversation_id_at_capture="original", captured_user_message_id="submitted-user")
    with pytest.raises(TurnReconciliationError, match="conversation_changed"):
...
@pytest.mark.asyncio
async def test_fresh_conversation_pins_as_soon_as_submitted_user_appears(monkeypatch):
    d = driver_for(monkeypatch, [], "")
    d._conversation_id_from_url = AsyncMock(side_effect=["server", "other"])
    d._fetch_text_for_turn = AsyncMock(return_value=TurnTextResult(
        "not_ready", diagnostic={"user_node": "submitted-user", "reason": "no_assistant_descendant"}))
    anchor = TurnAnchor(sent_text="request", mode="fresh_chat", captured_user_message_id="submitted-user")
    with pytest.raises(TurnReconciliationError, match="conversation_changed"):
        await read_protocol_reply(d, anchor, 1)
...
@pytest.mark.asyncio
async def test_fresh_unresolved_route_changes_are_bounded(monkeypatch):
    d = driver_for(monkeypatch, [], status="not_ready")
    d._conversation_id_from_url = AsyncMock(side_effect=["one", "two", "three", "four", "five"])
    anchor = TurnAnchor(sent_text="request", mode="fresh_chat", captured_user_message_id="submitted-user")
    with pytest.raises(TurnReconciliationError, match="conversation_changed"):
        await read_protocol_reply(d, anchor, 1)
    assert d._fetch_text_for_turn.await_count == 4
