...
async def test_cancelled_image_upload_skips_page_cleanup(monkeypatch):
    ...
    from tests.browser_fixtures import driver_for, image_part
    d = driver_for(monkeypatch, [], 'unused')
    # Block exactly at upload, then cancel before a send can occur.
    ...
    try:
        async def run():
            return [chunk async for chunk in d.send_and_stream('request', images=[decode_image(image_part())])]
